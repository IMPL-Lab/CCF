...
class PETRFormatBundle3D(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        if 'points_aug' in results:
            assert isinstance(results['points_aug'], BasePoints)
            results['points_aug'] = DC(results['points_aug'].tensor)
            
        if 'img_aug' in results:
            if isinstance(results['img_aug'], list):
                # process multiple imgs in single frame
                imgs = [img.transpose(2, 0, 1) for img in results['img_aug']]
                imgs = np.ascontiguousarray(np.stack(imgs, axis=0))
                results['img_aug'] = DC(to_tensor(imgs), stack=True)
            else:
                img = np.ascontiguousarray(results['img_aug'].transpose(2, 0, 1))
                results['img_aug'] = DC(to_tensor(img), stack=True)
