...
from .attention import FlashMHA, MultiScaleDeformableAttnWithQuery
...
class MV2DFusionTransformerDecoderLayer(BaseModule):
            ...
            if operation_name in attn_ops:
                if 'batch_first' in attn_cfgs[index]:
                    assert self.batch_first == attn_cfgs[index]['batch_first'] or attn_cfgs[index][
                        'type'] in ['PETRMultiheadFlashAttention']
                else:
    ...
    def _forward(self,
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                # Check whether modal weights should be saved
                save_modal_weights = kwargs.get('save_modal_weights', False)
                if save_modal_weights:
                    query = self.attentions[attn_index](
                        query.transpose(0, 1),
    ...
    def forward(self,
        ...
        if self.use_checkpoint and self.training:
            # cp.checkpoint does not support **kwargs, so save_modal_weights is passed manually.
            # Create a wrapper function to handle kwargs.
            def _forward_with_kwargs(*args):
...
class MV2DFusionTransformerDecoder(BaseModule):
    ...
    def forward(self, query, *args, query_pos=None, reference_points=None, dyn_q_coords=None, dyn_q_probs=None,
        ...
        for i, layer in enumerate(self.layers):
            if save_modal_weights:
                query = layer(query, *args, query_pos=query_pos, prev_ref_point=reference_points, 
                            save_modal_weights=save_modal_weights, **kwargs)
            else:
...
class MixedCrossAttention(BaseModule):
    def __init__(
            ...
            bev_norm=1,
            attn_cfg=None,
            if_deform_attn_with_query=False,
            img_only=False,
    ):
        ...
        self.if_deform_attn_with_query = if_deform_attn_with_query
        if if_deform_attn_with_query:
            self.deform_attn_with_query = MultiScaleDeformableAttnWithQuery(
    ...
    def forward(self, instance_feature, query_pos, reference_points, feat_flatten_img, spatial_flatten_img,
        ...
        if self.if_deform_attn_with_query:
            features_img = self.feature_sampling_img_with_query(
                instance_feature, feat_flatten_img, spatial_flatten_img,
                key_points, lidar2img_mat, img_metas)
        else:
    ...
    def feature_sampling_img_with_query(self, query, feat_flatten, spatial_flatten, key_points, lidar2img_mat, img_metas):
        # query: [B, n_q, C]
        ...
        # Reshape to match deformable attention expected format
        # [B * V, n_q, num_groups, num_levels, num_pts, 2]
        ...
        # Prepare query for attention computation
        # query needs to be expanded for each camera view
        # [B, n_q, C] -> [B * V, n_q, C]
        query_expanded = query.unsqueeze(1).repeat(1, self.num_cams, 1, 1).flatten(0, 1)
        # Reshape query to match num_groups and head_dims
        # [B * V, n_q, num_groups, group_dims]
        query_reshaped = query_expanded.reshape(bn, num_anchor, self.num_groups, self.group_dims)
        
        # Call multi_scale_deformable_attn_with_query
        # query: [bn, num_anchor, num_groups, group_dims]
        # value: [bn, num_value, num_groups, group_dims]
        # spatial_flatten: [num_levels, 2]
        # points_2d: [bn, num_anchor, num_groups, num_levels, num_pts, 2]
        output, attn_weights = self.deform_attn_with_query(
            query_reshaped, feat_flatten, spatial_flatten, points_2d)
        # output, attn_weights = multi_scale_deformable_attn_with_query(
        #     query_reshaped, feat_flatten, spatial_flatten, points_2d)

        # output: [bn, num_anchor, num_groups * group_dims]
...
@ATTENTION.register_module()
class MixedWeightedCrossAttention(BaseModule):
    def __init__(
            self,
            embed_dims=256,
            num_groups=8,
        ...
        super(MixedWeightedCrossAttention, self).__init__()
        self.embed_dims = embed_dims
        ...
        if self.separate_weight:
            input_dims = self.embed_dims * 3
            self.weight_net_img = nn.Sequential(
            ...
            self.weight_net_pts = nn.Sequential(
                nn.Linear(input_dims, self.embed_dims),
                nn.LayerNorm(self.embed_dims),
                nn.ReLU(inplace=False),
                nn.Linear(self.embed_dims, 1),
    ...
    def forward(self, instance_feature, query_pos, reference_points, feat_flatten_img, spatial_flatten_img,
                level_start_index_img, pc_range, lidar2img_mat, img_metas, feat_flatten_pts,
                pos_flatten_pts, save_modal_weights=False):
        ...
        features_img = self.feature_sampling_img(feat_flatten_img, spatial_flatten_img, level_start_index_img,
                                                 key_points, weights_img, lidar2img_mat, img_metas)
        output_img = self.output_proj_img(features_img)
        output_img = self.drop(output_img)
        ...
        if self.separate_weight:
            weight_img = self.weight_net_img(torch.cat([instance_feature, output_img, output_pts], dim=-1).detach())
            weight_pts = self.weight_net_pts(torch.cat([instance_feature, output_img, output_pts], dim=-1).detach())
            output = weight_img * output_img + weight_pts * output_pts + instance_feature
            # output = instance_feature + weight_pts * output_pts
            # output = instance_feature
        else:
            weight_img = self.weight_net(torch.cat([instance_feature, output_img, output_pts], dim=-1).detach())
            weight_pts = 1 - weight_img
            output = weight_img * output_img + weight_pts * output_pts + instance_feature

        # Save modal weights for supervision when requested.
        if save_modal_weights:
            # Store as instance attributes for later use.
            # weight_img: [B, num_queries, 1], image modality weights
            # weight_pts: [B, num_queries, 1], point cloud modality weights
            self._last_modal_weight_img = weight_img
            self._last_modal_weight_pts = weight_pts
