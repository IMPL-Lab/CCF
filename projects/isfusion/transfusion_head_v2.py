...
class TransFusionHeadV2(nn.Module):
    def __init__(self,
                 ...
                 # denosing
                 use_query_denoising= False, # new
                 noise_scale=1.0, # new
                 noise_trans=0.0, # new
                 add_gt_groups = 3, # new
                 denoise_query_num = 700, # new
                 ):
        super(TransFusionHeadV2, self).__init__()

        self.feat_modality = feat_modality
        self.use_query_denoising = use_query_denoising
        self.bbox_noise_scale = noise_scale
        self.bbox_noise_trans = noise_trans
        self.add_gt_groups = add_gt_groups
        self.denoise_query_num = denoise_query_num
        # self.fusion_layer = build_head(fusion_layer) if fusion_layer is not None else None
        self.num_proposals_ori = num_proposals
        self.center_decoder = build_head(center_decoder_cfg) # new
        self.center_head = MFSeparateHead(hidden_channel, center_head, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias) # new
        ...
        for i in range(self.num_decoder_layers):
            self.decoder.append(build_head(decoder_layer))
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        # Legacy nuScenes 10-class: softer pooling on pedestrian & traffic_cone channels.
        if (self.test_cfg["dataset"] == "nuScenes"
                and self.num_classes == 10):
            local_max[
            :,
            8,
        ...
        query_feat = self.center_decoder(
            query_feat,
        ...
        center_dict = self.center_head(query_feat)  
        
        query_pos += center_dict['center'].permute( 0, 2, 1 )   
        ...
        for i in range(self.num_decoder_layers):
            ...
            box_feat, cls_feat = self.decoder[i](
                query_feat,
                query_pos=query_pos,
                box_key=lidar_feat_flatten,
                box_key_pos=bev_pos,
                cls_key=lidar_feat_flatten,
                cls_key_pos=bev_pos,
                self_attn_mask = attn_mask, 
                )

            # Prediction
            res_layer = self.prediction_heads[i](box_feat, cls_feat)
            res_layer["center"] = query_pos.permute(0, 2, 1)
        ...
        # features for mv2df query generation
        ret_dicts[0]["lidar_feats"] = lidar_feat
        # ret_dicts[0]["query_xyz"] = query_pos
        ret_dicts[0]["query_cat"] = top_proposals_class
        ret_dicts[0]["query_feats"] = query_feat
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict, batch_idx):
        ...
        for idx_layer in range(num_layer):
            ...
            if self.train_cfg.assigner.type == "HungarianAssigner3DV2":
    ...
    def get_bboxes(self, preds_dicts, metas, img=None, rescale=False, for_roi=False, batch_id=0):
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            one_hot = F.one_hot(
                self.query_labels[batch_id].unsqueeze(0), num_classes=self.num_classes
            ).permute(0, 2, 1)
            ...
            if self.test_cfg["dataset"] == "nuScenes":
                custom_tasks = self.test_cfg.get("nms_tasks", None)
                if custom_tasks is not None:
                    self.tasks = custom_tasks
                    ...
                    per_radii = self.test_cfg.get("per_class_nms_radius", None)
                    if per_radii is None:
                        per_radii = [0.25] * self.num_classes
                    elif len(per_radii) < self.num_classes:
                        per_radii = list(per_radii) + [float(per_radii[-1])] * (
                            self.num_classes - len(per_radii))
                    self.tasks = [
                        dict(
                            num_class=1,
                            class_names=[],
                            indices=[i],
                            radius=float(per_radii[i]),
