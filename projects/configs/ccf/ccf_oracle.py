...
pts_ckpt = 'checkpoints/isfusion_oracle.pth'
img_ckpt = 'checkpoints/faster_rcnn_swint_fpn_oracle.pth'
...
model = dict(
    ...
    img_query_generator=dict(
        ...
        # extra_feat=[('pts_num', 1), ('box_h', 1), ('box_w', 1), ('cls_score', 11)],
...
train_pipeline = [
    ...
    dict(type='Collect3D', keys=['points', 'gt_bboxes_3d', 'gt_labels_3d', 'img', 'sparse_depth', 'gt_bboxes', 'gt_labels', 'centers2d', 'depths'] + collect_keys,
         meta_keys=(
             'filename', 'ori_shape', 'img_shape', 'pad_shape', 'scale_factor', 'flip', 'box_mode_3d', 'box_type_3d',
             'img_norm_cfg', 'gt_bboxes_3d', 'gt_labels_3d', 'gt_pts_visible_mask', 'gt_img_visible_mask'), )
]
test_pipeline = [
    dict(
        type='LoadPointsFromFile',
    ...
    dict(
        ...
        transforms=[
            dict(
                ...
                with_label=False),
            dict(type='Collect3D',
                 keys=['points', 'img', 'sparse_depth'] + collect_keys ,
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'img_roi_extractor': dict(lr_mult=0.1),
...
evaluation = dict(interval=8 * num_iters_per_epoch, pipeline=test_pipeline)
checkpoint_config = dict(interval=1 * num_iters_per_epoch)
find_unused_parameters = True
# runner = dict(
#     type='IterBasedRunner', max_iters=num_epochs * num_iters_per_epoch)
runner = dict(
    type='IterBasedRunnerWrapper', max_iters=num_epochs * num_iters_per_epoch)
load_from = None
resume_from = None
